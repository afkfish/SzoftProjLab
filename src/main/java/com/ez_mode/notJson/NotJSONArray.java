...
public class NotJSONArray implements Iterable<Object> {
  ...
  public <E extends Enum<E>> E getEnum(Class<E> clazz, int index) throws NotJSONException {
    ...
    if(val==null) {
      ...
      throw wrongValueFormatException(index, "enum of type "
              + NotJSONObject.quote(clazz.getSimpleName()), opt(index), null);
    }
    return val;
  }
  ...
  public String join(String separator) throws NotJSONException {
    ...
    StringBuilder sb = new StringBuilder(
            NotJSONObject.valueToString(this.myArrayList.get(0)));

    for (int i = 1; i < len; i++) {
      sb.append(separator)
              .append(NotJSONObject.valueToString(this.myArrayList.get(i)));
    }
    return sb.toString();
  }
  ...
  public String optString(int index, String defaultValue) {
    Object object = this.opt(index);
    return NotJSONObject.NULL.equals(object) ? defaultValue : object
            .toString();
  }
   ...
   * A NotNotJSONPointerException may be thrown from code called by this method.
   *
   ...
   *
   * <p>If <pre> {@code indentFactor > 0}</pre> and the {@link NotJSONArray} has only
  ...
  public Writer write(Writer writer, int indentFactor, int indent)
    ...
    try {
      ...
      if (length == 1) {
        try {
          NotJSONObject.writeValue(writer, this.myArrayList.get(0),
                  indentFactor, indent);
        } catch (Exception e) {
        ...
        for (int i = 0; i < length; i += 1) {
          ...
          try {
            NotJSONObject.writeValue(writer, this.myArrayList.get(i),
   ...
   *            A Collection.
   * @param wrap
   *            {@code true} to call {@link NotJSONObject#wrap(Object)} for each item,
