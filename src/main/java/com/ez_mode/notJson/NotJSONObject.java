...
public class NotJSONObject {
   ...
   */
  public NotJSONObject(NotJSONObject jo, String... names) {
    this(names.length);
  ...
  /**
   * Construct a JSONObject from a NotJSONTokener.
   *
   * @param x A NotJSONTokener object containing the source string.
   * @throws NotJSONException If there is a syntax error in the source string or a duplicated key.
   */
  public NotJSONObject(NotJSONTokener x) throws NotJSONException {
    this();
    char c;
    String key;
   ...
   * case.
   *
   * <p>Methods that are <code>static</code>, return <code>void</code>, have parameters, or are
   ...
   *
   * <p>The {@link NotJSONPropertyName} annotation can be used on a bean getter to override key name
   ...
   *
   * <p>Similarly, the {@link NotJSONPropertyName} annotation can be used on non- <code>get</code>
   * and <code>is</code> methods. We can also override key name used in the JSONObject as seen below
   ...
   * The resulting JSON object would contain <code>"FullName": "Larry Fine"</code>
   *
   * <p>The {@link NotJSONPropertyIgnore} annotation can be used to force the bean property to not
   * be serialized into JSON. If both {@link NotJSONPropertyIgnore} and {@link NotJSONPropertyName}
   * are defined on the same method, a depth comparison is performed and the one closest to the
   * concrete class being serialized is used. If both annotations are at the same level, then the
   * {@link NotJSONPropertyIgnore} annotation takes precedent and the field is not serialized. For
   ...
   */
  public NotJSONObject(Object bean) {
    this();
    this.populateMap(bean);
  }

  private NotJSONObject(Object bean, Set<Object> objectsRecord) {
    this();
   ...
   */
  public NotJSONObject(Object object, String... names) {
    this(names.length);
   ...
   * @exception NotJSONException If there is a syntax error in the source string or a duplicated
   *     key.
   */
  public NotJSONObject(String source) throws NotJSONException {
   ...
   * @throws NotJSONException If any NotJSONExceptions are detected.
   */
  public NotJSONObject(String baseName, Locale locale) throws NotJSONException {
    this();
    ...
    while (keys.hasMoreElements()) {
      ...
      if (key != null) {
        ...
        NotJSONObject target = this;
        for (int i = 0; i < last; i += 1) {
          String segment = path[i];
          NotJSONObject nextTarget = target.optJSONObject(segment);
          if (nextTarget == null) {
            nextTarget = new NotJSONObject();
   ...
   */
  protected NotJSONObject(int initialCapacity) {
    this.map = new HashMap<String, Object>(initialCapacity);
  }

  /**
   * Accumulate values under a key. It is similar to the put method except that if there is already
   * an object stored under the key then a NotJSONArray is stored under the key to hold all of the
   * accumulated values. If there is already a NotJSONArray, then the new value is appended to it.
   * In contrast, the put method replaces the previous value.
   *
   * <p>If only one value is accumulated that is not a NotJSONArray, then the result will be the
   * same as using put. But if multiple values are accumulated, then the result will be like append.
   *
   ...
   */
  public NotJSONObject accumulate(String key, Object value) throws NotJSONException {
    testValidity(value);
    Object object = this.opt(key);
    if (object == null) {
      this.put(key, value instanceof NotJSONArray ? new NotJSONArray().put(value) : value);
    } else if (object instanceof NotJSONArray) {
      ((NotJSONArray) object).put(value);
    } else {
      this.put(key, new NotJSONArray().put(object).put(value));
    }
    return this;
  }
   ...
   * key is put in the JSONObject with its value being a NotJSONArray containing the value
   * parameter. If the key was already associated with a NotJSONArray, then the value parameter is
   * appended to it.
   *
   * @param key A key string.
   * @param value An object to be accumulated under the key.
   * @return this.
   * @throws NotJSONException If the value is non-finite number or if the current value associated
   *     with the key is not a NotJSONArray.
   * @throws NullPointerException If the key is <code>null</code>.
   */
  public NotJSONObject append(String key, Object value) throws NotJSONException {
    testValidity(value);
    Object object = this.opt(key);
    if (object == null) {
      this.put(key, new NotJSONArray().put(value));
    } else if (object instanceof NotJSONArray) {
      this.put(key, ((NotJSONArray) object).put(value));
    } else {
      throw wrongValueFormatException(key, "NotJSONArray", null, null);
    }
    return this;
  }
   ...
   * @throws NotJSONException if the key is not found.
   */
  public Object get(String key) throws NotJSONException {
    if (key == null) {
      throw new NotJSONException("Null key.");
    }
    Object object = this.opt(key);
    if (object == null) {
      throw new NotJSONException("JSONObject[" + quote(key) + "] not found.");
    }
    return object;
  }
   ...
   *     enum.
   */
  public <E extends Enum<E>> E getEnum(Class<E> clazz, String key) throws NotJSONException {
    ...
    if (val == null) {
      ...
      throw wrongValueFormatException(
          key, "enum of type " + quote(clazz.getSimpleName()), opt(key), null);
    }
    return val;
  }
   ...
   * @param key A key string.
   * @return The truth.
   * @throws NotJSONException if the value is not a Boolean or the String "true" or "false".
   */
  public boolean getBoolean(String key) throws NotJSONException {
   ...
   * @throws NotJSONException if the key is not found or if the value cannot be converted to
   *     BigInteger.
   */
  public BigInteger getBigInteger(String key) throws NotJSONException {
    Object object = this.get(key);
  ...
  /**
   * Get the BigDecimal value associated with a key. If the value is float or double, the {@link
   ...
   *     BigDecimal.
   */
  public BigDecimal getBigDecimal(String key) throws NotJSONException {
    Object object = this.get(key);
   ...
   */
  public double getDouble(String key) throws NotJSONException {
   ...
   *     cannot be converted to a number.
   */
  public float getFloat(String key) throws NotJSONException {
    final Object object = this.get(key);
    if (object instanceof Number) {
   ...
   *     cannot be converted to a number.
   */
  public Number getNumber(String key) throws NotJSONException {
    Object object = this.get(key);
    try {
      if (object instanceof Number) {
   ...
   *     integer.
   */
  public int getInt(String key) throws NotJSONException {
    final Object object = this.get(key);
    if (object instanceof Number) {
  ...
  /**
   * Get the NotJSONArray value associated with a key.
   *
   * @param key A key string.
   * @return A NotJSONArray which is the value.
   * @throws NotJSONException if the key is not found or if the value is not a NotJSONArray.
   */
  public NotJSONArray getNotJSONArray(String key) throws NotJSONException {
    Object object = this.get(key);
    if (object instanceof NotJSONArray) {
      return (NotJSONArray) object;
    }
    throw wrongValueFormatException(key, "NotJSONArray", object, null);
  }
   ...
   * @throws NotJSONException if the key is not found or if the value is not a JSONObject.
   */
  public NotJSONObject getJSONObject(String key) throws NotJSONException {
    Object object = this.get(key);
   ...
   * @throws NotJSONException if the key is not found or if the value cannot be converted to a long.
   */
  public long getLong(String key) throws NotJSONException {
    final Object object = this.get(key);
    if (object instanceof Number) {
   ...
   */
  public static String[] getNames(NotJSONObject jo) {
    if (jo.isEmpty()) {
      return null;
    }
   ...
   * @throws NotJSONException if there is no string value for the key.
   */
  public String getString(String key) throws NotJSONException {
    Object object = this.get(key);
   ...
   * 1 (Integer). If there is such a property, and if it is an Integer, Long, Double, Float,
   ...
   * @param key A key string.
   * @return this.
   * @throws NotJSONException If there is already a property with this name that is not an Integer,
   *     Long, Double, or Float.
   */
  public NotJSONObject increment(String key) throws NotJSONException {
    ...
    if (value == null) {
      ...
      throw new NotJSONException("Unable to increment [" + quote(key) + "].");
    }
    return this;
  }
  ...
  public boolean isNull(String key) {
    return NotJSONObject.NULL.equals(this.opt(key));
  }
  ...
  /**
   * Get a set of keys of the JSONObject. Modifying this key Set will also modify the JSONObject.
   * Use with caution.
   *
   * @see Map#keySet()
   * @return A keySet.
   ...
   * contained therein will modify the backing JSONObject. This does not return a clone or a
   * read-only view.
   *
   * <p>Use with caution.
   *
  ...
  /**
   * Removes all of the elements from this JSONObject. The JSONObject will be empty after this call
   * returns.
   */
  public void clear() {
    this.map.clear();
  ...
  /**
   * Produce a NotJSONArray containing the names of the elements of this JSONObject.
   *
   * @return A NotJSONArray containing the key strings, or null if the JSONObject is empty.
   */
  public NotJSONArray names() {
    if (this.map.isEmpty()) {
      return null;
    }
    return new NotJSONArray(this.map.keySet());
  }
   ...
   * @return A String.
   * @throws NotJSONException If n is a non-finite number.
   */
  public static String numberToString(Number number) throws NotJSONException {
    if (number == null) {
   ...
   * @param val value to convert
   * @param defaultValue default value to return is the conversion doesn't work or is null.
   * @param exact When <code>true</code>, then {@link Double} and {@link Float} values will be
   *     converted exactly. When <code>false</code>, they will be converted to {@link String} values
   *     before converting to {@link BigDecimal}.
   * @return BigDecimal conversion of the original value, or the defaultValue if unable to convert.
   */
  ...
  /**
   * Get an optional NotJSONArray associated with a key. It returns null if there is no such key, or
   * if its value is not a NotJSONArray.
   *
   * @param key A key string.
   * @return A NotJSONArray which is the value.
   */
  public NotJSONArray optNotJSONArray(String key) {
   ...
   * @return A JSONObject which is the value.
   */
  public NotJSONObject optJSONObject(String key) {
    return this.optJSONObject(key, null);
  }

  /**
   * Get an optional JSONObject associated with a key, or the default if there is no such key or if
   ...
   */
  public NotJSONObject optJSONObject(String key, NotJSONObject defaultValue) {
    Object object = this.opt(key);
    return object instanceof NotJSONObject ? (NotJSONObject) object : defaultValue;
  }
   ...
   * recursive.
   *
   * @see NotJSONObject#NotJSONObject(Object)
   * @param bean the bean
   */
  ...
  private static String getKeyNameFromMethod(Method method) {
    final int ignoreDepth = getAnnotationDepth(method, NotJSONPropertyIgnore.class);
    if (ignoreDepth > 0) {
      final int forcedNameDepth = getAnnotationDepth(method, NotJSONPropertyName.class);
    ...
    NotJSONPropertyName annotation = getAnnotation(method, NotJSONPropertyName.class);
   ...
   * @return the {@link Annotation} if the annotation exists on the current method or one of its
   *     super class definitions
   */
  private static <A extends Annotation> A getAnnotation(
      final Method m, final Class<A> annotationClass) {
    ...
    try {
      return getAnnotation(
          c.getSuperclass().getMethod(m.getName(), m.getParameterTypes()), annotationClass);
    } catch (final SecurityException ex) {
      return null;
    } catch (final NoSuchMethodException ex) {
      return null;
  ...
  private static int getAnnotationDepth(
      final Method m, final Class<? extends Annotation> annotationClass) {
   ...
   * @throws NullPointerException If the key is <code>null</code>.
   */
  public NotJSONObject put(String key, boolean value) throws NotJSONException {
    return this.put(key, value ? Boolean.TRUE : Boolean.FALSE);
  }

  /**
   * Put a key/value pair in the JSONObject, where the value will be a NotJSONArray which is
   ...
   * @throws NullPointerException If the key is <code>null</code>.
   */
  public NotJSONObject put(String key, Collection<?> value) throws NotJSONException {
    return this.put(key, new NotJSONArray(value));
  }
   ...
   * @throws NullPointerException If the key is <code>null</code>.
   */
  public NotJSONObject put(String key, double value) throws NotJSONException {
   ...
   * @throws NullPointerException If the key is <code>null</code>.
   */
  public NotJSONObject put(String key, float value) throws NotJSONException {
   ...
   * @throws NullPointerException If the key is <code>null</code>.
   */
  public NotJSONObject put(String key, int value) throws NotJSONException {
   ...
   * @throws NullPointerException If the key is <code>null</code>.
   */
  public NotJSONObject put(String key, long value) throws NotJSONException {
   ...
   * @throws NullPointerException If the key is <code>null</code>.
   */
  public NotJSONObject put(String key, Map<?, ?> value) throws NotJSONException {
    return this.put(key, new NotJSONObject(value));
  }
   ...
   *     Integer, NotJSONArray, JSONObject, Long, String, or the JSONObject.NULL object.
   * @return this.
   * @throws NotJSONException If the value is non-finite number.
   * @throws NullPointerException If the key is <code>null</code>.
   */
  public NotJSONObject put(String key, Object value) throws NotJSONException {
    if (key == null) {
      throw new NullPointerException("Null key.");
    }
    if (value != null) {
   ...
   * @return this.
   * @throws NotJSONException if the key is a duplicate
   */
  public NotJSONObject putOnce(String key, Object value) throws NotJSONException {
    if (key != null && value != null) {
      if (this.opt(key) != null) {
        throw new NotJSONException("Duplicate key \"" + key + "\"");
      }
   ...
   *     Integer, NotJSONArray, JSONObject, Long, String, or the JSONObject.NULL object.
   * @return this.
   * @throws NotJSONException If the value is a non-finite number.
   */
  public NotJSONObject putOpt(String key, Object value) throws NotJSONException {
    if (key != null && value != null) {
      return this.put(key, value);
    }
    return this;
   ...
   * </pre>
   *
   * Then this method will return the String "c". A NotJSONPointerException may be thrown from code
  ...
  public boolean similar(Object other) {
    try {
      if (!(other instanceof NotJSONObject)) {
        return false;
      }
      if (!this.keySet().equals(((NotJSONObject) other).keySet())) {
        return false;
      }
      for (final Entry<String, ?> entry : this.entrySet()) {
        String name = entry.getKey();
        Object valueThis = entry.getValue();
        Object valueOther = ((NotJSONObject) other).get(name);
   ...
   *
   * <p>If either of the numbers are Double or Float instances, then they are checked to have a
   * finite value. If either value is not finite (NaN or &#177;infinity), then this function will
   * always return false. If both numbers are finite, they are first checked to be the same type and
   * implement {@link Comparable}. If they do, then the actual {@link Comparable#compareTo(Object)}
   * is called. If they are not the same type, or don't implement Comparable, then they are
   * converted to {@link BigDecimal}s. Finally the BigDecimal values are compared using {@link
   ...
   *     catch this and wrap it in a {@link NotJSONException} if applicable.
   */
   ...
   * @throws NotJSONException If o is a non-finite number.
   */
  public static void testValidity(Object o) throws NotJSONException {
    if (o instanceof Number && !numberIsFinite((Number) o)) {
      throw new NotJSONException("JSON does not allow non-finite numbers.");
    }
  }

  /**
   * Produce a NotJSONArray containing the values of the members of this JSONObject.
   *
   * @param names A NotJSONArray containing a list of key strings. This determines the sequence of
   *     the values in the result.
   * @return A NotJSONArray of values.
   * @throws NotJSONException If any of the values are non-finite numbers.
   */
  public NotJSONArray toNotJSONArray(NotJSONArray names) throws NotJSONException {
    if (names == null || names.isEmpty()) {
      return null;
    }
    NotJSONArray ja = new NotJSONArray();
    for (int i = 0; i < names.length(); i += 1) {
   ...
   *
   * and the {@link NotJSONObject} has only one key, then the object will be output on a single
   * line:
   *
   * <pre>{@code {"key": 1}}</pre>
   *
   * <p>If an object has 2 or more keys, then it will be output across multiple lines:
   *
   * <pre>{@code {
   *  "key1": 1,
   *  "key2": "value 2",
   ...
   *     beginning with <code>{</code>&nbsp;<small>(left brace)</small> and ending with <code>}
   *     </code>&nbsp;<small>(right brace)</small>.
   * @throws NotJSONException If the object contains an invalid number.
   */
  @SuppressWarnings("resource")
   ...
   * called. If the value is a MAP, then a JSONObject will be made from it and its toNotJSONString
  ...
  public static String valueToString(Object value) throws NotJSONException {
    ...
    return NotJSONWriter.valueToString(value);
  }

  /**
   * Wrap an object, if necessary. If the object is <code>null</code>, return the NULL object. If it
   * is an array or collection, wrap it in a NotJSONArray. If it is a map, wrap it in a JSONObject.
   * If it is a standard property (Double, String, et al) then it is already wrapped. Otherwise, if
   * it comes from one of the java packages, turn it into a string. And if it doesn't, try to wrap
   * it in a JSONObject. If the wrapping fails, then null is returned.
   *
  ...
  private static Object wrap(Object object, Set<Object> objectsRecord) {
    try {
      ...
      if (object instanceof NotJSONObject
          || object instanceof NotJSONArray
          || NULL.equals(object)
          || object instanceof NotJSONString
      ...
      if (object instanceof Collection) {
        ...
        return new NotJSONArray(coll);
      }
      if (object.getClass().isArray()) {
        return new NotJSONArray(object);
      }
      if (object instanceof Map) {
        Map<?, ?> map = (Map<?, ?>) object;
        return new NotJSONObject(map);
      }
      ...
      if (objectsRecord != null) {
        return new NotJSONObject(object, objectsRecord);
      }
      return new NotJSONObject(object);
  ...
  static final Writer writeValue(Writer writer, Object value, int indentFactor, int indent)
      throws NotJSONException, IOException {
    if (value == null || value.equals(null)) {
      writer.write("null");
    } else if (value instanceof NotJSONString) {
      Object o;
      try {
        o = ((NotJSONString) value).toJSONString();
      } catch (Exception e) {
    ...
    } else if (value instanceof NotJSONObject) {
      ((NotJSONObject) value).write(writer, indentFactor, indent);
    } else if (value instanceof NotJSONArray) {
      ((NotJSONArray) value).write(writer, indentFactor, indent);
    } else if (value instanceof Map) {
      Map<?, ?> map = (Map<?, ?>) value;
      new NotJSONObject(map).write(writer, indentFactor, indent);
    } else if (value instanceof Collection) {
      Collection<?> coll = (Collection<?>) value;
      new NotJSONArray(coll).write(writer, indentFactor, indent);
    } else if (value.getClass().isArray()) {
      new NotJSONArray(value).write(writer, indentFactor, indent);
    } else {
   ...
   * @return The writer.
   * @throws NotJSONException if a called function has an error or a write error occurs
   */
  @SuppressWarnings("resource")
    ...
    try {
      ...
      if (length == 1) {
        ...
        try {
          ...
          throw new NotJSONException("Unable to write JSONObject value for key: " + key, e);
        }
      } else if (length != 0) {
   ...
   * is a NotJSONArray or JSONObject it will also be converted.
   *
   * <p>Warning: This method assumes that the data structure is acyclical.
   *
  ...
  public Map<String, Object> toMap() {
    ...
    for (Entry<String, Object> entry : this.entrySet()) {
      ...
      if (entry.getValue() == null || NULL.equals(entry.getValue())) {
        value = null;
      } else if (entry.getValue() instanceof NotJSONObject) {
        value = ((NotJSONObject) entry.getValue()).toMap();
      } else if (entry.getValue() instanceof NotJSONArray) {
        value = ((NotJSONArray) entry.getValue()).toList();
      } else {
  ...
  private static NotJSONException wrongValueFormatException(
      String key, String valueType, Object value, Throwable cause) {
    if (value == null) {
  ...
  /**
   * Create a new NotJSONException in a common format for recursive object definition.
   *
   * @param key name of the key
   * @return NotJSONException that can be thrown.
   */
  private static NotJSONException recursivelyDefinedObjectException(String key) {
    return new NotJSONException(
