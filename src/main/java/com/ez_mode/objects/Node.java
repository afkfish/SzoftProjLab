...
public abstract class Node implements Tickable {
    ...
    public void addFlowRate(Node source, double excededFlow) {
        if (!this.sources.contains(source)) {
            ...
            this.absorbers.forEach(
                    standableObject -> standableObject.addFlowRate(this, excededFlow));
        }
    }
    ...
    public void removeFlowRate(Node source, double flowRate) {
        if (this.sources.contains(source)) {
            ...
            this.absorbers.forEach(
                    standableObject -> standableObject.removeFlowRate(this, flowRate));
        }
    }
    ...
    @Override
    public void tick() {
        assert this.connectors.size() <= this.maxConnections
    ...
    protected void calculateFlowRate() {
        ...
        if (this.isBroken || !this.connectors.stream().allMatch(Connector::isConnected)) {
            ...
            double waterLossPerConnection =
                    this.flowRate
                            * (looseConnectors / (this.connectors.size() - this.sources.size()));
    ...
    public void connect(Node node) throws ObjectFullException {
        Optional<Connector> freeConnector =
                this.connectors.stream().filter(connector -> !connector.isConnected()).findFirst();
